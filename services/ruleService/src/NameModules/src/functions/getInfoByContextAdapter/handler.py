...
@inject
def main(event, context, adapter: GetInfoByContextAdapter = Provide[Container.promo_adapter]):
    try:
...
container.wire(modules=["src.NameModules.src.functions.getInfoByContextAdapter.handler"])