...
class CriteriaEvaluator:
    ...
    def execute(self,label: str,criterions: List[ICriterion],context: Context,promo_name: str,rule: object,invert: bool = False) -> bool:
        print(f"► evaluating generic criterions for {label}:")
        for criterion in criterions:
            result = criterion.evaluate(context)
            final = not result if invert else result
            self.evaluation_recorder.execute(promo_name,str(rule),criterion.field_,criterion.operator_,str(criterion.value_),final)
            if not final:
                return False
        return True
